...
from models.common.bc_network import BCNetwork,InvFunction
...
class BCSSAgent(object):
    ...
    def init_bc_optimizers(self, bc_lr, bc_beta, train_state_encoder, train_policy, train_visual_encoder):
        ...
        self.bc_module_optimizer = torch.optim.AdamW(params=params, lr=bc_lr, betas=[0.9,0.999], weight_decay=self.weight_decay, eps=1.0e-6)
    ...
    def update_policy(self, concatenated_obs, action, robot_qpos, L=None, step=None):
        pred_action = self.bc_policy_network(concatenated_obs, robot_qpos)
    ...
    def update(self, obs=None, state=None, next_obs=None, next_state=None, action=None, robot_qpos=None, L=None, step=None, concatenated_obs=None, concatenated_next_obs=None):
        ...
        loss = self.update_policy(concatenated_obs, action, robot_qpos, L, step)
    ...
    def validate(self, obs=None, state=None, action=None, robot_qpos=None, L=None, step=None, concatenated_obs=None, mode='eval'):
        with torch.no_grad():
            if concatenated_obs is None:
                frame_stack = int(obs.shape[1]/3)
                assert self.frame_stack_num == frame_stack
