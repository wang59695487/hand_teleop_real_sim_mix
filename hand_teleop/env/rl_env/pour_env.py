...
class PourBoxRLEnv(PourBoxEnv, BaseRLEnv):
    def __init__(self, use_gui=False, frame_skip=5, robot_name="adroit_hand_free", constant_object_state=False,
                 object_seed=0, randomness_scale=1, friction=1, **renderer_kwargs):
        super().__init__(use_gui=use_gui, frame_skip=frame_skip, randomness_scale=randomness_scale, frictionf=friction,
                         object_seed=object_seed, **renderer_kwargs)
    ...
    def get_oracle_state(self):
        ...
        object_pose = self.bottle.get_pose()
    ...
    def _num_box_in_bowl(self):
        contact_buffer = self.check_actor_pair_contacts(self.boxes, self.bowl)
        return np.sum(contact_buffer)

    def _is_bottle_lifted(self):
        # check the x-y position of the object against the target
        object_z = self.bowl.pose.p[-1]
        if object_z >= 0.02:
            self.is_bottle_lifted = True

        return self.is_bottle_lifted

    def _is_bottle_still(self):
        # check if the object is close to still
        velocity_norm = np.linalg.norm(self.bottle.velocity)
        object_is_still = velocity_norm <= 1e-6

        return object_is_still

    def _is_success(self):
        return self._num_box_in_bowl() > 2 & self._is_bottle_lifted()

    def get_info(self):
        return {"num_box_in_bowl": self._num_box_in_bowl(),
                "is_bottle_lifted": self._is_bottle_lifted(),
...
def main_env():
    ...
    env = PourBoxRLEnv(use_gui=True, robot_name="allegro_hand_free",
