...
class PourBoxEnv(BaseSimulationEnv):
    def __init__(self, use_gui=True, frame_skip=5, object_seed=0, randomness_scale=1, friction=1, use_visual_obs=False,
                 init_obj_pos: Optional[sapien.Pose] = None, **renderer_kwargs):
        super().__init__(use_gui=use_gui, frame_skip=frame_skip, use_visual_obs=use_visual_obs, **renderer_kwargs)

        # Object info
        self.bottle_name = "pringles"
        ...
        self.np_random = np.random.RandomState(self.object_seed)
        ...
        # Load table
        # TODO: correct me with right table height
        self.tables = self.create_lab_tables(table_height=0.91)

        # Load bowl
        self.bowl_init_pose = sapien.Pose([0.0, 0.1, self.bowl_height])
        self.bowl = load_ycb_object(self.scene, self.bowl_name, static=True)
        self.bowl.set_pose(self.bowl_init_pose)
        ...
        builder.add_visual_from_file(str(visual_mesh), scale=[self.bottle_scale] * 3,
                                     pose=sapien.Pose(q=[0.707, 0.707, 0, 0]))
        builder.add_multiple_collisions_from_file(str(collision_mesh), scale=[self.bottle_scale] * 3,
                                                  pose=sapien.Pose(q=[0.707, 0.707, 0, 0]))
        self.bottle = builder.build(self.bottle_name)
    ...
    def generate_random_object_pose(self, randomness_scale):
        # TODO: correct me with right randomization params
        random = self.np_random
        pos_x = random.uniform(-0.05, 0.05) * randomness_scale
        pos_y = random.uniform(-0.1, -0.2) * randomness_scale
        position = np.array([pos_x, pos_y, 0])
        euler = random.uniform(-np.deg2rad(0), np.deg2rad(0))
    ...
    def reset_env(self):
        ...
        self.bottle.set_pose(init_pose)
        print(init_pose)
        for box in self.boxes:
    ...
    def load_box(self):
        box_material = self.scene.create_physical_material(1, 0.5, 0.01)
        colors = [np.array([1, 0, 0, 1]), np.array([0, 1, 0, 1]), np.array([0, 0, 1, 1]), np.array([1, 1, 0, 1])]
        box_half_len = 0.0125
        ...
        for i, color in enumerate(colors):
            builder = self.scene.create_actor_builder()
            builder.add_box_visual(half_size=box_size, color=color,
                                   pose=sapien.Pose([0, 0, box_len * i + 0.1]))
            builder.add_box_collision(half_size=box_size, material=box_material,
                                      pose=sapien.Pose([0, 0, box_len * i + 0.1]))
