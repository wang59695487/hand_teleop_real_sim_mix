...
class CupStackEnv(BaseSimulationEnv):
    def __init__(self, use_gui=True, frame_skip=5, object_category="YCB", object_name="tomato_soup_can", object_seed = 0,
        ...
        self.object_height = object_scale * YCB_SIZE["mug"][2] / 2
        ...
        self.scene_config.enable_tgs = True
        ...
        self.target_mug = load_ycb_object(self.scene, "big_blue_cup", scale=1, static=True)
        if init_target_pos is None:
            print('Using Default Target Location')
            self.target_pose = sapien.Pose([0, 0, 0])
        else:
            print('Using Given Target Location')
            self.target_pose = init_target_pos
        self.target_mug.set_pose(self.target_pose)

        # Load object
        if self.object_category.lower() == "ycb":
            self.manipulated_object = load_ycb_object(self.scene, "red_cup", scale=1, static=False)
    ...
    def generate_random_object_pose(self, randomness_scale):
        ...
        pos_x = random.uniform(-0.2, 0.2) * randomness_scale
        pos_y = random.uniform(-0.2, 0.2) * randomness_scale
    ...
    def generate_random_target_pose(self, randomness_scale):
        ...
        random_pose = sapien.Pose([-0.005, 0, 0.038])
    ...
    def generate_random_object_texture(self, randomness_scale):
        ...
        for visual in self.target_mug.get_visual_bodies():
...
def env_test():
    ...
    randomness_scale = 1
    env = CupStackEnv(object_name="tomato_soup_can", object_category="YCB", randomness_scale=randomness_scale, object_scale=0.8)
